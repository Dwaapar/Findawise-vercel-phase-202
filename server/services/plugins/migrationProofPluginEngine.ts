...
export class MigrationProofPluginEngine extends EventEmitter {
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      await this.startHealthMonitoringSafely();
      await this.startBackupSchedulerSafely();
      await this.startMigrationDetectionSafely();

      // Create initial backup with fallback
      await this.createEmergencyBackupSafely();

      // Register event handlers with error boundaries
      this.registerEventHandlersSafely();
  ...
  /**
   * Safe method implementations to prevent uncaught exceptions
   */
  private async startHealthMonitoringSafely(): Promise<void> {
    try {
  ...
  private async startBackupSchedulerSafely(): Promise<void> {
    try {
  ...
  private async startMigrationDetectionSafely(): Promise<void> {
    try {
      this.startMigrationDetection();
    } catch (error) {
      logger.error('Failed to start migration detection', {
        error: error?.message || 'Unknown error',
        component: 'MigrationProofPluginEngine'
      });
    }
  ...
  private async createEmergencyBackupSafely(): Promise<void> {
    try {
  ...
  private registerEventHandlersSafely(): void {
    try {
  ...
  private async enableFallbackMode(): Promise<void> {
    try {
      ...
      logger.info('Emergency fallback mode enabled', {
        component: 'MigrationProofPluginEngine'
      });
    } catch (error) {
    ...
    try {
      ...
      for (const plugin of plugins) {
        try {
          ...
          logger.debug('Plugin health check failed, continuing with others', {
            pluginId: plugin.id,
            component: 'MigrationProofPluginEngine'
          });
        }
      ...
      // Check overall system health safely
      await this.checkSystemHealthSafely();
    } catch (error) {
      // Continue operating in fallback mode
      logger.debug('Health checks completed with fallback mode', {
        component: 'MigrationProofPluginEngine'
      });
    }
  }
  ...
  private async checkSystemHealthSafely(): Promise<void> {
    try {
      await this.checkSystemHealth();
    } catch (error) {
      logger.debug('System health check failed, using fallback', {
        error: error?.message || 'Unknown error',
        component: 'MigrationProofPluginEngine'
      });
    }
  ...
  private async checkSystemHealth(): Promise<void> {
    // System health check implementation with error handling
    try {
  ...
  private async createEmergencyBackup(): Promise<void> {
    try {
      ...
      // Safely backup plugin states with error handling
      try {
        if (this.marketplace && typeof this.marketplace.getAvailablePlugins === 'function') {
          const pluginsResult = this.marketplace.getAvailablePlugins();
          // Add explicit check for iterable
          const plugins = Array.isArray(pluginsResult) ? pluginsResult : [];
          
          if (plugins.length > 0) {
            for (const plugin of plugins) {
              try {
                if (plugin && plugin.id && typeof this.marketplace.getPluginInstances === 'function') {
                ...
                // Plugin may not be available, continue with others
                logger.debug('Failed to backup plugin', { pluginId: plugin?.id || 'unknown', error: error?.message });
              }
            }
          } else {
            logger.debug('No plugins available for backup - using system-only backup');
          }
        } else {
          logger.debug('Marketplace not initialized - using system-only backup');
        }
      } catch (error) {
        logger.debug('Marketplace not available for backup - using system-only backup', { error: error?.message || 'plugins is not iterable' });
        // Continue with system-only backup - this is normal during startup
      }

      // Safely backup configurations
      try {
  ...
  private async checkMigrationIndicators(): Promise<{
    ...
    try {
      ...
      // Don't trigger migration mode for every system error
      // Only for critical errors that might indicate actual migration
      if (error.message && (
        ...
        confidence += 0.3; // Reduced confidence for general errors
      }
    }
  ...
  private async handleMigrationEvent(indicators: any): Promise<void> {
    ...
    try {
      // 1. Create immediate backup (with error handling)
      try {
        await this.createEmergencyBackup();
      } catch (error) {
        logger.warn('Emergency backup failed, continuing with other recovery steps', { error });
      }

      // 2. Switch to fallback mode
      await this.enableFallbackMode();

      // 3. Validate plugin integrity (skip if marketplace unavailable)
      try {
        await this.validatePluginIntegrity();
      } catch (error) {
        logger.warn('Plugin integrity validation failed, skipping', { error });
      }
      ...
      if (this.config.autoRecoveryEnabled) {
        try {
          ...
          logger.warn('Auto-recovery failed, system will continue in fallback mode', { error });
        }
      }

      // 5. Run emergency audit (skip if codex unavailable)
      try {
        await this.runEmergencyAudit();
      } catch (error) {
        logger.warn('Emergency audit failed, skipping', { error });
      }
  ...
  /**
   * System Health Checks - Enhanced Implementation
   */
  private async checkSystemHealthEnhanced(): Promise<void> {
