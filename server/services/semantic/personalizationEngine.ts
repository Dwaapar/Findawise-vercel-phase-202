...
export class PersonalizationEngine {
  ...
  async getUserIntentVector(
    ...
    try {
      ...
      if (userId) {
        result = await db.select().from(userIntentVectors)
          .where(eq(userIntentVectors.userId, userId))
          .limit(1);
      } else if (sessionId) {
        result = await db.select().from(userIntentVectors)
          .where(eq(userIntentVectors.sessionId, sessionId))
          .limit(1);
      } else if (fingerprint) {
        result = await db.select().from(userIntentVectors)
          .where(eq(userIntentVectors.fingerprint, fingerprint))
          .limit(1);
      } else {
        result = await db.select().from(userIntentVectors).limit(1);
      }
