...
export class ProductionSeedEngine {
  ...
  private async seedCoreNeurons(): Promise<SeedResult> {
    ...
    for (const neuron of coreNeurons) {
      try {
        ...
        // Create the neuron
        await this.storage.registerNeuron(neuron);
        result.created++;
  ...
  private async seedSystemUsers(): Promise<SeedResult> {
    ...
    for (const user of systemUsers) {
      try {
        ...
        const existing = await this.storage.getGlobalUserProfileByEmail(user.email);
  ...
  private async seedCoreConfiguration(): Promise<SeedResult> {
    ...
    for (const config of coreConfigs) {
      try {
        // Note: setConfig method doesn't exist, skipping for now
        console.log(`✅ Config seeding skipped: ${config.key}`);
        result.created++;
      } catch (error) {
  ...
  private async seedSemanticGraph(): Promise<SeedResult> {
    ...
    try {
      ...
      // Note: initializeSemanticGraph method doesn't exist, skipping for now
      if (false) {
