...
Last updated: ${new Date().toISOString().split('T')[0]}`;
  ...
  async healthCheck() {
    try {
      ...
      const frameworks = COMPLIANCE_FRAMEWORKS.length;
      
      // Test storage operations
      const testUserId = 'health-check-test';
      const canRead = await this.storage.getGlobalConsentsByUser(testUserId).catch(() => []);
      
      // Check if compliance tables are accessible
      const tablesAccessible = Array.isArray(canRead);
      ...
      return {
        ...
        features: {
          ...
          privacyPolicyGeneration: true,
          affiliateCompliance: true,
      ...
      return {
        ...
        features: {
          ...
          privacyPolicyGeneration: true, // Still works without DB
          affiliateCompliance: false,
          geoRestrictions: true, // Static data
          userDataRequests: false,
          auditSystem: false
        }
      };
    }
