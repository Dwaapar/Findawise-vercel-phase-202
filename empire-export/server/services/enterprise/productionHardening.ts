...
class ProductionHardening {
  ...
  constructor() {
    this.config = {
      ...
      maxMemoryUsage: 90, // Increased threshold to 90% to reduce alerts
      maxCpuUsage: 85, // Increased threshold to 85%
      maxConnections: 1000,
  ...
  private startHealthMonitoring(): void {
    ...
    const optimizedInterval = Math.max(this.config.healthCheckInterval, 120000); // Minimum 2 minutes
    
    setInterval(async () => {
      await this.performHealthChecks();
    }, optimizedInterval);

    console.log('🏥 Health monitoring started (optimized interval)');
  }
  ...
  private async performHealthChecks(): Promise<void> {
    ...
    for (const component of components) {
      try {
        ...
        // Auto-restart if threshold exceeded with aggressive rate limiting
        if (errorCount >= this.config.autoRestartThreshold) {
          const lastFailureTime = this.lastFailureTime.get(component.name) || 0;
          const timeSinceLastFailure = Date.now() - lastFailureTime;
          
          // Increase cooldown to 5 minutes to reduce system stress
          if (timeSinceLastFailure > 300000) { // 5 minutes
            await this.handleComponentFailure(component.name);
            this.lastFailureTime.set(component.name, Date.now());
          }
        }
  ...
  private async handleComponentFailure(component: string): Promise<void> {
    ...
    try {
      switch (component) {
        case 'websocket':
          // Safe websocket restart - check if restart method exists
          if (webSocketManager && typeof webSocketManager.restart === 'function') {
            await webSocketManager.restart();
          } else {
            console.log('🔄 Websocket manager restart not available, using fallback');
            // Initialize or reinitialize websocket manager if needed
          }
          break;
        case 'ai-services':
          // Restart AI services with memory cleanup
        ...
        case 'federation':
          ...
          break;
        case 'web-server':
          // For web-server component, perform memory optimization instead of restart
          console.log('🔄 Optimizing web server performance...');
  ...
  private startSystemMonitoring(): void {
    ...
    }, 60000); // Reduced to every 60 seconds to lower system load
